...
import md5 from 'md5';
// import { urlConfigs } from './constants';
...
class Identify {
	...
	static getSimiCartConfig() {
		return {
			// merchant_url: urlConfigs.merchantUrl,
			// api_path: '',
			// simicart_url: urlConfigs.simicartUrl,
			// simicart_authorization: urlConfigs.tokenKey,
		};
	}
	...
	static formatPrice(price, type = 0) {
		...
		if (merchant_config !== null) {
			let currency_symbol =
				merchant_config.storeview.base.currency_symbol ||
				merchant_config.storeview.base.currency_code;
			let currency_position =
				merchant_config.storeview.base.currency_position;
			let decimal_separator =
				merchant_config.storeview.base.decimal_separator;
			let thousand_separator =
				merchant_config.storeview.base.thousand_separator;
			let max_number_of_decimals =
				merchant_config.storeview.base.max_number_of_decimals;
			if (type === 1) {
				return Identify.putThousandsSeparators(
					price,
					thousand_separator,
					decimal_separator,
			...
			if (currency_position == 'before') {
				return (
					currency_symbol +
					Identify.putThousandsSeparators(
						price,
						thousand_separator,
						decimal_separator,
						max_number_of_decimals,
	...
	static putThousandsSeparators(value, sep, decimal, max_number_of_decimals) {
		if (!max_number_of_decimals) {
			let merchant_config = this.getMerchantConfig();
			max_number_of_decimals =
				merchant_config.storeview.base.max_number_of_decimals || 2;
		}
	...
	static isPluginEnabled(sku) {
		...
		if (
			typeof this.pluginsStatus[sku] === 'undefined' &&
	...
	static getCheckoutParams(redirectUrl, quoteId) {
		...
		let encodedCustomerData = new Buffer(
			JSON.stringify(customerData),
		).toString('base64');

		secretKey =
			secretKey.slice(0, salt.length) +
			salt +
			secretKey.slice(salt.length, secretKey.length);
		token =
			encodedCustomerData.slice(0, salt.length) +
			secretKey +
			encodedCustomerData.slice(salt.length, encodedCustomerData.length);
	...
	static isMagento2() {
		...
		if (
			...
			merchant_config.storeview.base.magento_version &&
			merchant_config.storeview.base.magento_version == '2'
		) {
			return true;
		}
		return false;
